...
/*
 * この underscore.php は、CakePHP 2.x とのクラス宣言衝突回避のため、
 * アンダースコア２つから、アンダースコア３つに変更してあります。
 * 動作確認が取れていないため、様子を見て使ってくださいな。
 *
 */
...
function ___($item=null) {
  $___ = new ___;
  if(func_num_args() > 0) $___->_wrapped = $item;
  return $___;
}
...
class ___ {
  ...
  public function chain($item=null) {
    ...
    $___ = (isset($this) && isset($this->_chained) && $this->_chained) ? $this : ___($item);
    $___->_chained = true;
    return $___;
  }
  ...
  public function reduceRight($collection=null, $iterator=null, $memo=null) {
    ...
    $___ = new self;
    return self::_wrap($___->reduce($collection, $iterator, $memo));
  }
  ...
  public function invoke($collection=null, $function_name=null, $arguments=null) {
    ...
    $___ = new self;
    list($collection, $function_name) = $___->first($args, 2);
    $arguments = $___->rest(func_get_args(), 2);
  ...
  public function any($collection=null, $iterator=null) {
    ...
    $___ = new self;
    if(!is_null($iterator)) $collection = $___->map($collection, $iterator);
  ...
  public function initial($collection=null, $n=null) {
    ...
    $___ = new self;
    return self::_wrap($___->first($collection, $first_index));
  }
  ...
  public function compact($collection=null) {
    ...
    $___ = new self;
    return self::_wrap($___->select($collection, function($val) {
      return (bool) $val;
    }));
  }
  ...
  public function flatten($collection=null, $shallow=null) {
    ...
    if(count($collection) > 0) {
      foreach($collection as $item) {
        if(is_array($item)) {
          $___ = new self;
          $return = array_merge($return, ($shallow) ? $item : $___->flatten($item));
        }
        else $return[] = $item;
      }
    }
  ...
  public function intersection($array=null) {
    ...
    $___ = new self;
    $return = $___->first($arrays);
    foreach($___->rest($arrays) as $next) {
      if(!$___->isArray($next)) $next = str_split((string) $next);
  ...
  public function union($array=null) {
    ...
    $___ = new self;
    return self::_wrap($___->flatten(array_values(array_unique(call_user_func_array('array_merge', $arrays)))));
  }
  ...
  public function lastIndexOf($collection=null, $item=null) {
    ...
    krsort($collection);
    $___ = new self;
    return self::_wrap($___->indexOf($collection, $item));
  }
  ...
  public function range($stop=null) {
    ...
    $___ = new self;
    $args = $___->reject($args, function($val) {
      return is_null($val);
    });
    ...
    if($step > 0 && $___->last($results) >= $stop) array_pop($results);
    elseif($step < 0 && $___->last($results) <= $stop) array_pop($results);
  ...
  public function zip($array=null) {
    ...
    $___ = new self;
    $num_return_arrays = $___->max($___->map($arrays, function($array) {
      return count($array);
    }));
    $return_arrays = $___->range($num_return_arrays);
  ...
  public function max($collection=null, $iterator=null) {
    ...
    arsort($results);
    $___ = new self;
    $first_key = $___->first(array_keys($results));
  ...
  public function extend($object=null) {
    ...
    $array = (array) $object;
    $___ = new self;
    $extensions = $___->rest(func_get_args());
  ...
  public function isEqual($a=null, $b=null) {
    ...
    if(is_object($a) || is_array($a)) {
      ...
      $___ = new self;
      $keys_equal = $___->isEqual($___->keys($a), $___->keys($b));
      $values_equal = $___->isEqual($___->values($a), $___->values($b));
